...
/// returning a mesh keeps its creator alive. The material and mesh headers are
/// included because TPZApproxCreator.h only forward declares them.
/// TPZMHMHDivApproxCreator::InsertMaterialObjects is declared but never defined
/// in NeoPZ, so it cannot be bound
...
void InitApproxCreator(py::module_ &m)
{
     py::class_<TPZHybrid>(m, "TPZHybrid",
                           "Geometric elements involved in one hybridization.")
         .def(py::init<>())
         .def(py::init<int64_t, int64_t, int64_t>(),
              py::arg("lagrange"), py::arg("left"), py::arg("right"))
         .def_readwrite("fLagrange", &TPZHybrid::fLagrange)
         .def_readwrite("fLeft", &TPZHybrid::fLeft)
         .def_readwrite("fRight", &TPZHybrid::fRight);

     auto approxCreator = py::class_<TPZApproxCreator>(
         m, "TPZApproxCreator", "Manages the construction of approximation spaces.");

     py::class_<TPZApproxCreator::HybridizationData>(
         approxCreator, "HybridizationData",
         "Material ids and multipliers describing a hybridization. The list "
         "and dict fields convert by copy: read, change, assign back.")
         .def(py::init<>())
         .def(py::init<const TPZApproxCreator::HybridizationData &>(), py::arg("other"))
         .def("__copy__", [](const TPZApproxCreator::HybridizationData &data)
              { return TPZApproxCreator::HybridizationData(data); })
         .def_readwrite("fWrapMatId",
                        &TPZApproxCreator::HybridizationData::fWrapMatId)
         .def_readwrite("fLeftInterfaceMatId",
                        &TPZApproxCreator::HybridizationData::fLeftInterfaceMatId)
         .def_readwrite("fRightInterfaceMatId",
                        &TPZApproxCreator::HybridizationData::fRightInterfaceMatId)
         .def_readwrite("fLagrangeMatId",
                        &TPZApproxCreator::HybridizationData::fLagrangeMatId)
         .def_readwrite("fSecondLeftInterfaceMatId",
                        &TPZApproxCreator::HybridizationData::fSecondLeftInterfaceMatId)
         .def_readwrite("fSecondRightInterfaceMatId",
                        &TPZApproxCreator::HybridizationData::fSecondRightInterfaceMatId)
         .def_readwrite("fSecondLagrangeMatId",
                        &TPZApproxCreator::HybridizationData::fSecondLagrangeMatId)
         .def_readwrite("fHybridizeBCLevel",
                        &TPZApproxCreator::HybridizationData::fHybridizeBCLevel)
         .def_readwrite("fMultipliers",
                        &TPZApproxCreator::HybridizationData::fMultipliers)
         .def_readwrite("fInterfaces",
                        &TPZApproxCreator::HybridizationData::fInterfaces)
         .def("SetProblemHybridH1",
              &TPZApproxCreator::HybridizationData::SetProblemHybridH1,
              py::arg("prob"), py::arg("hybrid"))
         .def("SetProblemHybridHDiv",
              &TPZApproxCreator::HybridizationData::SetProblemHybridHDiv,
              py::arg("prob"), py::arg("hybrid"))
         .def("__str__", [](const TPZApproxCreator::HybridizationData &data)
              { return PrintToString(data); });
     ...
     approxCreator
         ...
         .def("SetExtraInternalOrder", &TPZApproxCreator::SetExtraInternalOrder,
              py::arg("order"), "0 is HDiv, 1 HDiv+, 2 HDiv++.")
         .def("GetExtraInternalOrder", &TPZApproxCreator::GetExtraInternalOrder)
         .def("SetShouldCondense", &TPZApproxCreator::SetShouldCondense,
              py::arg("isCondensed"))
         /// GetShouldCondense takes an argument it ignores
         .def("ShouldCondense", [](TPZApproxCreator &creator)
              { return creator.GetShouldCondense(false); })
         .def_property("IsRigidBodySpaces", [](const TPZApproxCreator &creator)
                       { return creator.IsRigidBodySpaces(); }, [](TPZApproxCreator &creator, bool value)
                       { creator.IsRigidBodySpaces() = value; }, "Enriches the space with constant fields.")
         ...
         .def("__str__", [](TPZApproxCreator &creator)
              { return PrintToString(creator); });

     py::class_<TPZH1ApproxCreator, TPZApproxCreator>(
         m, "TPZH1ApproxCreator", "Approximation spaces based on H1 spaces.")
         .def(py::init<>())
         .def(py::init<TPZGeoMesh *>(), py::arg("gmesh"), py::keep_alive<1, 2>())
         .def("CreateClassicH1ApproximationSpace",
              &TPZH1ApproxCreator::CreateClassicH1ApproximationSpace,
              py::keep_alive<0, 1>(),
              "Single H1 mesh. Only valid without hybridization.")
     ...
     py::class_<TPZHDivApproxCreator, TPZApproxCreator>(
         m, "TPZHDivApproxCreator", "Approximation spaces based on HDiv spaces.")
         .def(py::init<>())
         .def(py::init<TPZGeoMesh *>(), py::arg("gmesh"), py::keep_alive<1, 2>())
         .def_property("HdivFamily", [](const TPZHDivApproxCreator &self)
                       { return self.HdivFamily(); }, [](TPZHDivApproxCreator &self, HDivFamily family)
                       { self.HdivFamily() = family; })
     ...
     py::class_<TPZMHMApproxCreator>(m, "TPZMHMApproxCreator",
                                     "Element partition used by the MHM creators.")
         .def(py::init<>())
         .def(py::init([](TPZVec<int64_t> elementPartition)
                       { return new TPZMHMApproxCreator(elementPartition); }),
              py::arg("elementPartition"));

     py::class_<TPZMHMHDivApproxCreator, TPZHDivApproxCreator, TPZMHMApproxCreator>(
         m, "TPZMHMHDivApproxCreator", "MHM variant of the HDiv creator.")
         .def(py::init<TPZGeoMesh *>(), py::arg("gmesh"), py::keep_alive<1, 2>())
         .def(py::init([](TPZGeoMesh *gmesh, TPZVec<int64_t> elementPartition)
                       { return new TPZMHMHDivApproxCreator(gmesh, elementPartition); }),
              py::arg("gmesh"), py::arg("elementPartition"), py::keep_alive<1, 2>())
         .def("BuildMultiphysicsCMesh",
              &TPZMHMHDivApproxCreator::BuildMultiphysicsCMesh, py::keep_alive<0, 1>(),
              "MHM driver. Call SetPOrderSkeleton first: the C++ constructors "
              "leave that member uninitialized.")
         .def("PutinSubstructures", &TPZMHMHDivApproxCreator::PutinSubstructures,
              py::arg("cmesh"))
         .def("CondenseElements", &TPZMHMHDivApproxCreator::CondenseElements,
              py::arg("cmesh"))
         .def("SetPOrderSkeleton", &TPZMHMHDivApproxCreator::SetPOrderSkeleton,
              py::arg("order"))
